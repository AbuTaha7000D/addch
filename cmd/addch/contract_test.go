...
func TestContractParseSingleFile(t *testing.T) {
	tests := []struct {
		...
		{"both positionals", []string{"chapters.txt", "video.mp4"},
			func(p *parsedArgs) bool { return p.chapters == "chapters.txt" && p.video == "video.mp4" }},
		{"short -o", []string{"-o", "out.mp4", "chapters.txt", "video.mp4"},
			func(p *parsedArgs) bool { return p.output == "out.mp4" }},
		{"long --output", []string{"--output", "out.mp4", "chapters.txt", "video.mp4"},
			func(p *parsedArgs) bool { return p.output == "out.mp4" }},
...
func TestContractUsageErrorsExit1(t *testing.T) {
	tests := []struct {
		...
		{"too many arguments", []string{"a.txt", "b.mp4", "c.txt"}, "too many arguments"},
