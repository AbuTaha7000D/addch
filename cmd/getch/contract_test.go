...
func TestContractParseSingleFile(t *testing.T) {
	tests := []struct {
		...
		{"short -o", []string{"-o", "out.txt", "video.mp4"},
			func(p *parsedArgs) bool { return p.output == "out.txt" && p.video == "video.mp4" }},
		{"long --output", []string{"--output", "out.txt", "video.mp4"},
			func(p *parsedArgs) bool { return p.output == "out.txt" && p.video == "video.mp4" }},
...
func TestContractUsageErrorsExit1(t *testing.T) {
	tests := []struct {
		...
		{"dir with output", []string{"--dir", "--output", "out.txt"}, "--output cannot be combined with"},
		{"recursive with output", []string{"--recursive", "-o", "out.txt"}, "--output cannot be combined with"},
		{"overwrite without output", []string{"--overwrite", "video.mp4"}, "--overwrite requires --output"},
	}
	for _, tc := range tests {
		var out, errBuf bytes.Buffer
